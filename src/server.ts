...
async function startServer() {
  ...
  const serverHttp = http.createServer((req, res) => {
          ...
          try {
            ...
            if (sessionId && transports[sessionId]) {
              ...
              await sessionServer.connect(
                transport as Parameters<typeof sessionServer.connect>[0]
              );
            } else if (!sessionId) {
              const statelessServer = createMcpServer();
              transport = new StreamableHTTPServerTransport();
              await statelessServer.connect(
                transport as Parameters<typeof statelessServer.connect>[0]
              );
            } else {
              res.writeHead(400, { "Content-Type": "application/json" });
              res.end(
    ...
    if (req.url === "/mcp" && req.method === "GET") {
      ...
      transports[sessionId].handleRequest(req, res).catch((error: unknown) => {
